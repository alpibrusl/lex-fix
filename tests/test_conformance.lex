...
fn valid_nos_message() -> msg.FixMessage {
  let n := { cl_ord_id: "ORD-001", symbol: "MSFT", side: Buy(()), order_qty: 100, ord_type: Limit(()), price: Some("125.50"), time_in_force: Day(()), transact_time: "20260527-09:30:00.000", sender_comp_id: "ALGO01", target_comp_id: "EXCH01", account: None }
...
fn valid_ocr_message() -> msg.FixMessage {
  ocr.to_fix_message(ocr.cancel_request("ORD-002", "ORD-001", "MSFT", Buy(()), 100, "20260528-10:00:00.000", "ALGO01", "EXCH01", None, None), 2)
}
...
fn valid_ocrr_message() -> msg.FixMessage {
  ocrr.to_fix_message(ocrr.cancel_replace_request("ORD-003", "ORD-001", "MSFT", Buy(()), 150, Limit(()), Some("124.00"), Day(()), "20260528-10:01:00.000", "ALGO01", "EXCH01", None), 3)
}
...
fn test_replace_market_no_price_ok() -> Result[Unit, Str] {
  let m := ocrr.to_fix_message(ocrr.cancel_replace_request("ORD-004", "ORD-001", "MSFT", Sell(()), 200, Market(()), None, Day(()), "20260528-10:02:00.000", "ALGO01", "EXCH01", None), 4)
...
fn valid_osr_message() -> msg.FixMessage {
  osr.to_fix_message(osr.status_request("ORD-001", "MSFT", Buy(()), None, "ALGO01", "EXCH01", None), 5, "20260528-10:03:00.000")
}
...
fn valid_ocrej_message() -> msg.FixMessage {
  ocrej.to_fix_message(ocrej.cancel_reject("ORD-002", "ORD-001", None, StatusNew(()), ResponseToCancel(()), None, "EXCH01", "ALGO01", None), 6, "20260528-10:04:00.000")
}
...
fn test_cancel_reject_missing_response_to_fails() -> Result[Unit, Str] {
  ...
  let fields := list.fold(base.fields, [], fn (acc :: List[field.FixField], f :: field.FixField) -> List[field.FixField] {
    if f.tag == tag.cxl_rej_response_to() {
      acc
    } else {
      list.concat(acc, [f])
    }
...
fn test_replace_same_side_passes() -> Result[Unit, Str] {
  let orig := valid_nos_message()
  let replace := valid_ocrr_message()
...
fn test_replace_changes_side_fails() -> Result[Unit, Str] {
  let orig := valid_nos_message()
  let flipped := ocrr.to_fix_message(ocrr.cancel_replace_request("ORD-003", "ORD-001", "MSFT", Sell(()), 150, Limit(()), Some("124.00"), Day(()), "20260528-10:01:00.000", "ALGO01", "EXCH01", None), 3)
...
fn test_replace_changes_symbol_fails() -> Result[Unit, Str] {
  let orig := valid_nos_message()
  let resymbol := ocrr.to_fix_message(ocrr.cancel_replace_request("ORD-003", "ORD-001", "AAPL", Buy(()), 150, Limit(()), Some("124.00"), Day(()), "20260528-10:01:00.000", "ALGO01", "EXCH01", None), 3)
...
fn suite() -> List[Result[Unit, Str]] {
  [test_valid_order_passes(), test_missing_symbol_fails(), test_invalid_side_fails(), test_limit_without_price_fails(), test_valid_cancel_passes(), test_cancel_missing_orig_fails(), test_cancel_wrong_msg_type_fails(), test_valid_replace_passes(), test_replace_limit_without_price_fails(), test_replace_market_no_price_ok(), test_valid_status_passes(), test_status_missing_side_fails(), test_valid_cancel_reject_passes(), test_cancel_reject_missing_response_to_fails(), test_cancel_reject_invalid_response_to_fails(), test_replace_same_side_passes(), test_replace_changes_side_fails(), test_replace_changes_symbol_fails()]
}
