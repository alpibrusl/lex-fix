...
fn sample_ocr() -> ocr.OrderCancelRequest {
  ocr.cancel_request("ORD-002", "ORD-001", "MSFT", Buy(()), 100, "20260528-10:00:00.000", "ALGO01", "EXCH01", Some("ACC-A"), None)
}

fn sample_ocrr() -> ocrr.OrderCancelReplaceRequest {
  ocrr.cancel_replace_request("ORD-003", "ORD-001", "MSFT", Buy(()), 150, Limit(()), Some("124.00"), Day(()), "20260528-10:01:00.000", "ALGO01", "EXCH01", Some("ACC-A"))
}

fn sample_osr() -> osr.OrderStatusRequest {
  osr.status_request("ORD-001", "MSFT", Buy(()), Some("EXCH-ID-001"), "ALGO01", "EXCH01", None)
}

fn sample_ocrej() -> ocrej.OrderCancelReject {
  ocrej.cancel_reject("ORD-002", "ORD-001", Some("EXCH-ID-001"), StatusNew(()), ResponseToCancel(()), Some(TooLateToCancel(())), "EXCH01", "ALGO01", Some("order already filled"))
}
...
fn test_ocrr_roundtrip() -> Result[Unit, Str] {
  ...
  match ocrr.from_fix_message(m) {
    Err(_) => fail("roundtrip parse failed"),
    Ok(r) => assert_true(r.cl_ord_id == "ORD-003" and r.orig_cl_ord_id == "ORD-001", "cl_ord_id and orig_cl_ord_id roundtrip"),
  }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [test_ocr_msg_type(), test_ocr_orig_cl_ord_id(), test_ocr_side(), test_ocr_account_present(), test_ocr_no_text(), test_ocr_roundtrip(), test_ocr_qty_roundtrip(), test_ocrr_msg_type(), test_ocrr_orig_cl_ord_id(), test_ocrr_price_present(), test_ocrr_ord_type(), test_ocrr_roundtrip(), test_ocrr_qty_roundtrip(), test_osr_msg_type(), test_osr_order_id_present(), test_osr_no_account(), test_osr_roundtrip(), test_ocrej_msg_type(), test_ocrej_cxl_rej_response_to(), test_ocrej_reason_present(), test_ocrej_text_present(), test_ocrej_roundtrip()]
}
