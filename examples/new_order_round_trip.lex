...
fn make_limit_order() -> nos.NewOrderSingle {
  { cl_ord_id: "ORD-2026-001", symbol: "MSFT", side: Buy(()), order_qty: 100, ord_type: Limit(()), price: Some("125.50"), time_in_force: Day(()), transact_time: "20260527-09:30:00.000", sender_comp_id: "ALGO01", target_comp_id: "EXCH01", account: None }
}

fn make_invalid_order() -> nos.NewOrderSingle {
  { cl_ord_id: "ORD-2026-002", symbol: "AAPL", side: Sell(()), order_qty: 50, ord_type: Limit(()), price: None, time_in_force: Gtc(()), transact_time: "20260527-09:30:01.000", sender_comp_id: "ALGO01", target_comp_id: "EXCH01", account: Some("ACCT-001") }
}
...
fn run_valid() -> Str {
  ...
  match conf.validate_new_order(fix_msg) {
    ...
    Err(es) => str.concat("unexpected rejection: ", list.fold(conf.describe_errors(es), "", fn (acc :: Str, e :: Str) -> Str {
      if acc == "" {
        e
      } else {
        str.concat(acc, str.concat("; ", e))
      }
    })),
  }
}
...
fn run_invalid() -> Str {
  ...
  match conf.validate_new_order(fix_msg) {
    ...
    Err(es) => str.concat("correctly rejected — violations: ", list.fold(conf.describe_errors(es), "", fn (acc :: Str, e :: Str) -> Str {
      if acc == "" {
        e
      } else {
        str.concat(acc, str.concat("; ", e))
