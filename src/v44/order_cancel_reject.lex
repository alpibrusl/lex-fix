...
# ---- Typed domain record ----------------------------------------
type OrderCancelReject = { cl_ord_id :: Str, orig_cl_ord_id :: Str, order_id :: Option[Str], ord_status :: en.OrdStatus, cxl_rej_response_to :: en.CxlRejResponseTo, cxl_rej_reason :: Option[en.CxlRejReason], sender_comp_id :: Str, target_comp_id :: Str, text :: Option[Str] }

fn cancel_reject(cl_ord_id :: Str, orig_cl_ord_id :: Str, order_id :: Option[Str], ord_status :: en.OrdStatus, cxl_rej_response_to :: en.CxlRejResponseTo, cxl_rej_reason :: Option[en.CxlRejReason], sender_comp_id :: Str, target_comp_id :: Str, text :: Option[Str]) -> OrderCancelReject {
  { cl_ord_id: cl_ord_id, orig_cl_ord_id: orig_cl_ord_id, order_id: order_id, ord_status: ord_status, cxl_rej_response_to: cxl_rej_response_to, cxl_rej_reason: cxl_rej_reason, sender_comp_id: sender_comp_id, target_comp_id: target_comp_id, text: text }
}

# ---- Typed → FixMessage -----------------------------------------
fn to_fix_message(ocrej :: OrderCancelReject, seq_num :: Int, sending_time :: Str) -> msg.FixMessage {
  let header_fields := msg.header(tag.mt_order_cancel_reject(), ocrej.sender_comp_id, ocrej.target_comp_id, int.to_str(seq_num), sending_time)
  let body_fields := [{ tag: tag.cl_ord_id(), value: ocrej.cl_ord_id }, { tag: tag.orig_cl_ord_id(), value: ocrej.orig_cl_ord_id }, { tag: tag.ord_status(), value: en.ord_status_to_str(ocrej.ord_status) }, { tag: tag.cxl_rej_response_to(), value: en.cxl_rej_response_to_to_str(ocrej.cxl_rej_response_to) }]
              ...
              Some(crt) => match field.require(fields, tag.sender_comp_id()) {
                ...
                Ok(sndr) => match field.require(fields, tag.target_comp_id()) {
                  ...
                  Ok(tgt) => {
                    ...
                    Ok({ cl_ord_id: coi, orig_cl_ord_id: ocoi, order_id: field.get(fields, tag.order_id()), ord_status: os, cxl_rej_response_to: crt, cxl_rej_reason: crr, sender_comp_id: sndr, target_comp_id: tgt, text: field.get(fields, tag.text()) })
                  },
                },
              },
            },
