...
# ---- Typed domain record -----------------------------------------
type NewOrderSingle = { cl_ord_id :: Str, symbol :: Str, side :: en.Side, order_qty :: Int, ord_type :: en.OrdType, price :: Option[Str], time_in_force :: en.TimeInForce, transact_time :: Str, sender_comp_id :: Str, target_comp_id :: Str, account :: Option[Str] }

fn new_order(cl_ord_id :: Str, symbol :: Str, side :: en.Side, order_qty :: Int, ord_type :: en.OrdType, price :: Option[Str], time_in_force :: en.TimeInForce, transact_time :: Str, sender_comp_id :: Str, target_comp_id :: Str, account :: Option[Str]) -> NewOrderSingle {
  { cl_ord_id: cl_ord_id, symbol: symbol, side: side, order_qty: order_qty, ord_type: ord_type, price: price, time_in_force: time_in_force, transact_time: transact_time, sender_comp_id: sender_comp_id, target_comp_id: target_comp_id, account: account }
}

# ---- Typed → FixMessage ------------------------------------------
fn to_fix_message(nos :: NewOrderSingle, seq_num :: Int) -> msg.FixMessage {
  let header_fields := msg.header(tag.mt_new_order_single(), nos.sender_comp_id, nos.target_comp_id, int.to_str(seq_num), nos.transact_time)
  let body_fields := [{ tag: tag.cl_ord_id(), value: nos.cl_ord_id }, { tag: tag.symbol(), value: nos.symbol }, { tag: tag.side(), value: en.side_to_str(nos.side) }, { tag: tag.order_qty(), value: int.to_str(nos.order_qty) }, { tag: tag.ord_type(), value: en.ord_type_to_str(nos.ord_type) }, { tag: tag.time_in_force(), value: en.tif_to_str(nos.time_in_force) }, { tag: tag.transact_time(), value: nos.transact_time }]
                      ...
                      Ok(sndr) => match field.require(fields, tag.target_comp_id()) {
                        ...
                        Ok(tgt) => match field.require(fields, tag.order_qty()) {
                          ...
                          Ok(qty_s) => match str.to_int(qty_s) {
                            ...
                            Some(qty) => Ok({ cl_ord_id: coi, symbol: sym, side: s, order_qty: qty, ord_type: ot, price: field.get(fields, tag.price()), time_in_force: tif, transact_time: tt, sender_comp_id: sndr, target_comp_id: tgt, account: field.get(fields, tag.account()) }),
                          },
                        },
                      },
                    },
