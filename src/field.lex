...
type FixField = { tag :: Int, value :: Str }
...
{
  list.fold(fields, None, fn (acc :: Option[Str], f :: FixField) -> Option[Str] {
    match acc {
      Some(_) => acc,
      None => if f.tag == t {
        Some(f.value)
...
fn set(fields :: List[FixField], t :: Int, v :: Str) -> List[FixField] {
  let rest := list.fold(fields, [], fn (acc :: List[FixField], f :: FixField) -> List[FixField] {
    if f.tag != t {
      list.concat(acc, [f])
    } else {
      acc
...
fn validate_one_of(fields :: List[FixField], t :: Int, allowed :: List[Str]) -> Option[e.FixError] {
  match get(fields, t) {
    None => None,
    Some(v) => {
      let found := list.fold(allowed, false, fn (acc :: Bool, a :: Str) -> Bool {
        if acc {
          true
        } else {
          v == a
